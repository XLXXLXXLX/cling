...
namespace cling {
...
#endif
  {
    ...
    m_BackendPasses.reset(new BackendPasses(CI.getCodeGenOpts(), *m_JIT,
                                            m_JIT->getTargetMachine()));
  }
  ...
  void* IncrementalExecutor::HandleMissingFunction(
...
freeCallersOfUnresolvedSymbols(llvm::SmallVectorImpl<llvm::Function*>&
  ...
  void* IncrementalExecutor::getAddressOfGlobal(llvm::StringRef symbolName,
    ...
    // FIXME: If we split the loaded libraries into a separate JITDylib we
    // should be able to delete this code and use something like:
...
#if !defined(_WIN32)
      void* Addr =
          llvm::sys::DynamicLibrary::SearchForAddressOfSymbol(symbolName.str());
#else
  ...
  bool
  IncrementalExecutor::diagnoseUnresolvedSymbols(llvm::StringRef trigger,
                                                 llvm::StringRef title) const {
    if (m_unresolvedSymbols.empty())
      return false;
    if (m_unresolvedSymbols.size() == 1 &&
        *m_unresolvedSymbols.begin() == trigger)
      return false;
    ...
    //m_Diags.Report(diagID) << sym << funcname; // TODO: demangle the names.
      ...
      if (trigger.find(utils::Synthesize::UniquePrefix) !=
          llvm::StringRef::npos)
      ...
      if (!demangledName.empty()) {
        cling::errs()
            << "You are probably missing the definition of " << demangledName
            << "\n"
            << "Maybe you need to load the corresponding shared library?\n";
      }

      std::string libName =
          m_DyLibManager.searchLibrariesForSymbol(sym,
                                                  /*searchSystem=*/true);
      if (!libName.empty())
        cling::errs() << "Symbol found in '" << libName << "';"
                      << " did you mean to load it with '.L " << libName
                      << "'?\n";
